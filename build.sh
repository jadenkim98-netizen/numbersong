...
rue_data = "data:image/png;base64," + base64.b64encode(open("keepers/keeper_rue_south.png","rb").read()).decode()  # Rue's cutscene sprite (minor tutorial)
sylva_data = "data:image/png;base64," + base64.b64encode(open("keepers/keeper_sylva_south.png","rb").read()).decode()  # Sylva's cutscene sprite (chords tutorial)
...
# Outer Keys props (PixelLab): the boat moored at each world's dock, and the Colour Guard.
boat_data = "data:image/png;base64," + base64.b64encode(open("adventure/w2_art/boat.png","rb").read()).decode()
shield_data = "data:image/png;base64," + base64.b64encode(open("adventure/w2_art/shield.png","rb").read()).decode()
...
<script>
  ...
  window.BOAT_SPRITE = "{boat_data}";
  window.SHIELD_SPRITE = "{shield_data}";
...
<script>
  ...
  // Home-screen apps (iOS especially) rarely reload: "opening" the app resumes the page
  // already in memory, so a new deploy never shows up. On every return to the foreground,
  // compare this page's build stamp with the live sw.js. Away 5+ minutes → reload quietly;
  // back sooner (maybe mid-question) → offer a one-tap banner instead of yanking the page.
  (function () {{
    var BUILD = "__NS_BUILD__", hiddenAt = 0, AWAY_MS = 5 * 60 * 1000;
    function banner() {{
      if (document.getElementById("ns-update")) return;
      var b = document.createElement("button");
      b.id = "ns-update";
      b.textContent = "New version ready · tap to update";
      b.style.cssText = "position:fixed;left:50%;transform:translateX(-50%);bottom:calc(16px + env(safe-area-inset-bottom,0px));z-index:9999;" +
        "font:600 14px system-ui,sans-serif;padding:12px 18px;border:2px solid #20302E;background:#57C6C4;color:#20302E;cursor:pointer;box-shadow:0 4px 0 rgba(0,0,0,.35);";
    ...
    document.addEventListener("visibilitychange", function () {{
      ...
      check(hiddenAt ? Date.now() - hiddenAt : 0);
    }});
    window.__nsCheckUpdate = check; // lets a test drive it
  }})();
</script>
</body>
</html>
...
# The page carries the same stamp, so it can tell when a newer sw.js is live (see the
# update check near the end of index.html).
sed -i.bak "s/__NS_BUILD__/$SW_VERSION/" index.html && rm -f index.html.bak
